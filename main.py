...
class FormMenu:
    ...
    def _init_menu(self):
        ...
        # transaction menu
        transaction = Menu(self.menu)
        self.menu.add_cascade(label="Transactions", menu=transaction)
        transaction.add_command(label="Sales", command=self.addinvoice_click)
        transaction.add_command(label="Sales Return", command=self.about_click)
        transaction.add_command(label="Purchase bill", command=self.about_click)
        transaction.add_command(label="Estimate", command=self.about_click)

        # edit menu
        edit = Menu(self.menu)
        self.menu.add_cascade(label="Edit", menu=edit)
        edit.add_command(label="Purchase Order", command=self.products_click)
        edit.add_command(label="Opening Stock", command=self.about_click)
        edit.add_command(label="Stock Adjustment", command=self.about_click)

        # report menu
        report = Menu(self.menu)
        self.menu.add_cascade(label="Report", menu=report)
        report.add_command(label="Customers", command=self.about_click)
        report.add_command(label="Purchase Report", command=self.about_click)
        report.add_command(label="Stock Report", command=self.about_click)
        report.add_command(label="GST Report", command=self.about_click)
    ...
    def addinvoice_click(self):        
        ...
        self.frm_invoices.frame.wm_transient(self.frame)
        # self.frame.wait_window(self.frm_invoices.frame)
        # self.frame.deiconify()
